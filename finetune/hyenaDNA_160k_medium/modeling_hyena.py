...
from typing import Optional, Tuple, Union, Unpack, Sequence, Dict, Any
...
class HyenaLMBackbone(nn.Module):
    ...
    def forward(self, input_ids, inputs_embeds=None, output_hidden_states=False, soft_prompt=None):
        all_hidden_states = []
        ...
        if soft_prompt is not None: hidden_states = torch.cat([soft_prompt, hidden_states], dim=1)
...
class HyenaDNAModel(HyenaDNAPreTrainedModel):
    ...
    def forward(self, input_ids, inputs_embeds=None, output_hidden_states=None, return_dict=None, soft_prompt=None):
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        )
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict

        hidden_states, all_hidden_states = self.backbone(input_ids, inputs_embeds=inputs_embeds, output_hidden_states=output_hidden_states, soft_prompt=soft_prompt)
        if return_dict:
...
class HyenaDNAForCausalLM(HyenaDNAPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config, **kwargs):
        ...
        if self.config.num_prompts > 0:
            ...
            nn.init.xavier_normal_(self.softPrompt)
        else:
    ...
    def prepare_inputs_for_generation(
        self,
        input_ids: torch.Tensor=None, inputs_embeds=None, prompt_idx: torch.Tensor=None, **kwargs
    ):
        if inputs_embeds is not None:
        ...
        model_inputs.update({'prompt_idx': prompt_idx})
        return model_inputs
        ...
        **kwargs: Unpack[Dict]
        ...
        if labels is not None:
            ...
            if prompt_idx is not None:
                shift_logits = shift_logits[..., self.config.prompt_size:, :]
